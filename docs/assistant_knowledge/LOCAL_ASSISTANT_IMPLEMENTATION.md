...
球体采用半透明全息风格：球心低不透明度，无实体暗面和镜面高光；柔和轮廓、三层羽化流动光带及轨道粒子由 `AuroraOrbGraphic` 直接绘制 UI 网格。对话框采用半透明深蓝底和淡青蓝紫细边，正文保持清晰。没有新增图片依赖，也不需要外部 Shader。使用项目已有 `simfang.ttf` 显示中文。
