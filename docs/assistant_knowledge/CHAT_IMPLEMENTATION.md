...
后续更新：第三批定位与当前操作说明均已接通，协议版本为 `assistant-chat-v3-guidance`。下文的“本批”描述保留第二批范围；当前流程、新增字段和验收方式以 [GUIDANCE_IMPLEMENTATION.md](GUIDANCE_IMPLEMENTATION.md) 为准。

## 当前流程

运行主实验场景 → H 或点击小球 → 本地问候 → 点击「问点什么」→ 输入问题 → 点击「发送」→ 本地后端加载项目资料并调用 DeepSeek → 检查回复 → 在问答窗口显示。

问候、闲置提醒、打开问答窗口、选择主题和输入文字均不调用 API。点击发送才请求 DeepSeek；语音输入在录音结束后独立请求 Groq 转写。服务错误不会显示成固定知识拒答。
...
顶部提供五个主题示例，点击只填入问题，不自动发送。VR 可使用现有 UI 射线点击这些示例和按钮，并使用「屏幕键盘」输入英文、数字及基本标点；屏幕键盘不是中文拼音输入法。PC 使用系统输入法输入中文。新增「语音输入」支持 Groq 中英文原语言转写，填入草稿后仍需确认发送；启动、配置与测试见 [VOICE_IMPLEMENTATION.md](VOICE_IMPLEMENTATION.md)。
